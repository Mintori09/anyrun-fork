...
fn run_swaymsg(args: &[&str]) -> Option<String> {
...
fn parse_windows(tree_json: &str) -> Vec<WindowInfo> {
    let root: TreeNode = match serde_json::from_str(tree_json) {
        Ok(node) => node,
        Err(e) => {
            eprintln!("[window-switcher] Failed to parse sway tree output: {e}");
            return Vec::new();
        }
    };
...
impl WindowBackend for SwayBackend {
    fn list_windows(&self) -> Vec<WindowInfo> {
        let tree = match run_swaymsg(&["-t", "get_tree"]) {
            Some(tree) => tree,
            None => return Vec::new(),
        };
        parse_windows(&tree)
...
mod tests {
    ...
    #[test]
    fn parse_sway_tree_uses_workspace_and_app_id() {
        let json = r#"{
            "id": 1,
            "name": "root",
            "nodes": [{
        ...
        assert_eq!(windows[0].id, "100");
        assert_eq!(windows[0].title, "Code");
        assert_eq!(windows[0].workspace.as_deref(), Some("1"));
        assert_eq!(windows[0].app_id.as_deref(), Some("code"));
    }
}
